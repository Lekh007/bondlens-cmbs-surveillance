...
This is a separate portfolio project from BondLens:

- **BondLens** demonstrates a LangGraph financial surveillance agent using structured SEC data and deterministic analytical tools.
- **InvestRAG Studio** demonstrates extensive LangChain usage, heterogeneous document processing, portable and database-native retrieval, reranking, provenance, and controlled evaluation.
...
7. Large source artifacts, model caches, and benchmark artifacts live on `D:`. Docker-managed database data uses named volumes rather than depending on Windows bind-mount performance.
8. The backend uses LangChain Runnables and retrievers. It does not use LangGraph, preserving a clear distinction from BondLens.
