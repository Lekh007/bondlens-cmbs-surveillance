...
The [README](README.md) is the short version. This is the long one: what each
tool in the stack actually proved, every bug that only real data could surface,
and the gaps that are still open.

For the design record see [`docs/plans/design.md`](docs/plans/design.md); for the
evaluation itself see
[`docs/demo/bondlens-golden-questions.md`](docs/demo/bondlens-golden-questions.md).
